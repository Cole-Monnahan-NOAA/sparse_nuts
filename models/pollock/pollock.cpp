...
Type objective_function<Type>::operator() ()
{
  ...
  for (i=0; i<nyears_proj; i++) {
    ...
    loglik(22) += dnorm(log_slp2_srv1, Type(-1.0),Type(.5), true);
    // loglik(22) += dnorm(inf1_srv1, Type(0.0),Type(3.0), true);
    loglik(22) += dnorm(inf2_srv1, Type(10.0),Type(2.0), true);
    loglik(22) += dnorm(log_slp1_srv2, Type(-1.0),Type(.5), true);
    ...
    loglik(22) += dnorm(log_slp1_srv6, Type(-1.0),Type(1), true);
    loglik(22) += dnorm(log_slp2_srv6, Type(-1.0),Type(1), true);
  ...
  loglik(22) += dnorm(transf_rho, Type(0),Type(.5), true);
  objfun = -sum(loglik);
